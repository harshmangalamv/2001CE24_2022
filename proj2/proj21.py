...
#         continue
        ...
        for row in range(3, rowMax + 4):
            ...
            for i in range(mn, mx):
                ...
                if(df.at[i, "Octant"] in freq):
                    freq[df.at[i, "Octant"]] = freq[df.at[i, "Octant"]] + \
                        1  # incrementing the key value for the key
                else:
                    # initiating the key if not present
    ...
    #   print("runs3")
        # stores the last time stamp for longest sequence
        store = [[], [], [], [], [], [], [], []]
        ...
        for i in range(1, 9):
            ...
            for j in range(1, len(store[loc[mp[i]]]) + 1):
                df.loc[row + j+1, "lsl."] = store[loc[mp[i]]][j - 1] - \
                    (int(df.at[int(loc[mp[i]])+2,
                     "Longest Subsequence Length"]) - 1)*0.01
